...
for kd=1:Nd
    if abs(P(1,detinfo(1,kd))-0)<1e-8 % left
        if abs(P(2,detinfo(1,kd))-0)<1e-8
            detinfo(2,kd)=14;
        elseif abs(P(2,detinfo(1,kd))-2)<1e-8
            detinfo(2,kd)=3;
        else
            detinfo(2,kd)=4;
        end
    elseif abs(P(1,detinfo(1,kd))-2)<1e-8 % right
        if abs(P(2,detinfo(1,kd))-0)<1e-8
            detinfo(2,kd)=12;
        elseif abs(P(2,detinfo(1,kd))-2)<1e-8
            detinfo(2,kd)=3;
...
for kd=1:Nd
    ...
    if abs(P(1,detinfo(1,kd))-2)<1e-8 & abs(P(2,detinfo(1,kd))-0)<1e-8
        srcdetpair(:,kd)=0; % take out the (2,0) corner from detector list
    end
    if abs(P(1,detinfo(1,kd))-2)<1e-8 & abs(P(2,detinfo(1,kd))-2)<1e-8
        srcdetpair(:,kd)=0; % take out the (2,2) corner from detector list
    end
    if abs(P(1,detinfo(1,kd))-0)<1e-8 & abs(P(2,detinfo(1,kd))-2)<1e-8
